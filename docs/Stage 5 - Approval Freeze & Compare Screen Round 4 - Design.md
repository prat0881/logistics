...
| --- | --- | --- | --- |
| `Quote.draftJson` | `FfPortalService.saveDraft` (any writable status), and `submit` (seeded with what was just sent) | The forwarder's **scratchpad**. Their last saved state, and nothing more. May be half-typed, blank, or a revision they never sent. | The FF portal only — `resolveScope` serves it as the portal's `draft`, which is the pre-fill, preview and print view the forwarder sees; **and `FfPortalService.submit`, which reads this column to build the authoritative draft it then writes to `submittedJson` — that read is how the scratchpad becomes the offer, and it is the only place the two columns touch.** Cleared for an `RFQ_SENT` quote by the expiry sweep and by `RfqService.distribute`; cleared for a refreshing quote by the change-order re-freeze. |
