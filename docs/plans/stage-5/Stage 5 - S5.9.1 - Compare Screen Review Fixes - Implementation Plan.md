...
- **Async-auth test trap:** `renderWithProviders`' `/api/auth/me` stub resolves asynchronously, so an assertion made synchronously after `render()` fires while `user` is `null` and proves nothing. Await a positive control first — the `AuthProbe` pattern, now in `ComparisonGrid.test.tsx` (~lines 254-260, mounted via `renderPanel`'s `withAuthProbe` option) since `CheckerPanel.test.tsx` was renamed to `GenerateGate.test.tsx` in this sub-build. **Every absence assertion must be mutation-proven** (break it → red → revert → green) and the evidence reported.
