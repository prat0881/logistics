...
- **Commits are controller-only (git-drift hazard):** a background process intermittently `git checkout main`s this shared checkout, so **implementers must not touch git** (no add/commit/stash/checkout). Each implementer reports the exact files + a suggested message; the controller commits after `git checkout feat/stage-3-cargo-packing-list` and verifies the parent. This **overrides every per-task "Commit" step** below.
...
- **Migrations:** Prisma cannot express the **generated column** or an **enum `ADD VALUE`** → hand-author `prisma/migrations/<ts>_<name>/migration.sql`. **Apply with `prisma migrate deploy` — NEVER `migrate dev`** (it errors PG 42601 on the generated `volumeCbm` column and may propose a destructive RESET of the shared :5433 dev DB). Export env first (`set -a; . apps/api/.env; set +a`), then `migrate deploy` + `prisma generate`. Do NOT run `prisma format` (reformats unrelated models → churn).
...
> **⚠ REVISED (pre-flight — this supersedes Steps 4–5's in-place-rename SQL and `migrate dev`; see design §11):** the migration is **DROP + CREATE** the cargo cluster, applied via **`prisma migrate deploy` — never `migrate dev`** (it errors PG 42601 on the generated column and may propose a destructive reset of the shared :5433 DB). Sequence: **(a) controller runs a confirmed one-time `TRUNCATE` of transactional tables** (queries + downstream; keep masters/config/catalogue); **(b) migration DDL** = create enums (`ALTER TYPE … ADD VALUE` not referenced later) → `DROP TABLE "LegCargo"`, `DELETE FROM "QuoteCargoLine"` + drop its cargo FK + rename `cargoItemId→packageId`, `DROP TABLE "CargoItem"` → `CREATE TABLE "Cargo"`, `"Package"` (carries the generated `volumeCbm` + `@@unique(queryId,packageNo)`), `"Item"`, `"LegPackage"` → re-add `QuoteCargoLine.packageId` FK → `Package`; **(c) `migrate deploy` + `prisma generate` + zero-drift check**. Steps 1–3 (test, Prisma models) and 6 (run test) stand as written; author the SQL from the Step-3 schema + §11. Verify the actual `QuoteCargoLine` cargo-FK constraint name in the DB before dropping it.
