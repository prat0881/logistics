...
Write `prisma/migrations/20260826094000_warehouse_master/migration.sql`:
...
The two helpers, defined at the top of the file:
...
Both names are prefixed `Linking` so the `afterAll` cleanup can delete them with a `startsWith` filter, following `vessels.e2e-spec.ts:38`.
