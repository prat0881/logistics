...
8. **Guard the `REOPEN` fire to distributed leg states (pre-existing edge, found in the whole-branch review).** The apply saga reopens the legs that carry a live (`RFQ_SENT|QUOTED`) quote. But a **second consecutive change-order** on a leg that was already reopened and **not yet re-distributed** — which still has a pending `RFQ_SENT` FF, so it re-qualifies — fires `REOPEN` on a leg now in `READY_FOR_RFQ`; because SB6 reuses the Stage-3 `REOPEN` event, the Stage-3 `READY_FOR_RFQ → DRAFT` edge fires and silently regresses the leg to `DRAFT`. Narrow (needs two change-orders with no re-distribute between) and non-blocking, but real. **Fix:** before firing, filter the reopen set to legs whose *current* status is in `{RFQ_SENT, PARTIALLY_QUOTED, FULLY_QUOTED}` (skip legs already at `READY_FOR_RFQ`/`DRAFT`).
