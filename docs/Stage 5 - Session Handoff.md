...
_Last updated: 2026-08-22 (S5.6 + S5.7 + S5.8 + **S5.9 all COMPLETE**. **All open points are consolidated in the register below** — start there.)_

## Current stage & branch
- **Stage 5 — Compare Quotes & Award.** Branch `feat/stage-5-fx-master` → **PR #52** (OPEN, **not merged** — you merge manually).
- S5.9 (Approval Flow & Compare Screen Rework, 10 tasks) landed on top of S5.4–S5.8: `docs(s5.9): design of record + implementation plan` (`13625c7`) through Task 10's tooltip move + this handoff. Code tip through Task 9: `b3834e5`.
- `pnpm run ci` **GREEN**: shared 26 files/390 tests · web 113 files/781 tests · api 93 suites/442 tests · lint + typecheck + 3 builds clean. Full gate re-run clean with **no flake** this session (see register **C5** for the known intermittent one).
...
| **C8** | **NEW (S5.9) — a live forwarder portal link is readable by any authenticated user, not just Managers/Admins.** `GET /api/queries/:id/emails` returns `bodyRendered` — the rendered `rfq.invitation` email, including the working `{{Access_Link}}` — with **no `@Roles`** on the route, for **any** query, not just ones the caller works on. This predates S5.9 entirely; it surfaced during S5.9 T5's review of D7/D8 (below) and is recorded here as its own item because it is real, unrelated to those decisions, and out of scope for this sub-build to fix. It compounds with D7/D8: since S5.9 the portal token no longer rotates and its raw value is persisted (`Rfq.accessToken`), so a link read off this endpoint now stays live indefinitely rather than being replaced on the next re-quote. `Rfq.accessToken` itself was reviewed and reaches no other DTO/response/log (every `Rfq` serialization is hand-mapped) — the exposure is entirely through `bodyRendered`, not through the token field. **Needs your call on priority** — the fix is a small one (`@Roles` on the emails route, or stop echoing `bodyRendered` to non-privileged callers), but it's a genuine access-control gap on a route that hands out a working third-party link. |
...
- **B1, B2 and B3 — closed by S5.9.**
  - **B1** (negotiate block at `PENDING_APPROVAL` was UI-only): closed as a side effect of S5.9 introducing a real `PENDING_APPROVAL` **quote** status, not just a decision status. `negotiation.service.ts`'s `REQUOTABLE_STATUSES = [QUOTED, APPROVED]` was left unchanged — it didn't need to change, because the shortlisted quote itself now genuinely leaves `QUOTED` the moment it's sent for approval, so the server already refuses to re-quote it. PO ruling **D5** confirmed the intended behaviour (negotiate stays refused while pending approval) and the UI (`NegotiateDialog`'s eligibility list, `CompareLegPanel`'s disabled-reason text) now matches the server exactly instead of contradicting it.
...
- **The `MessageLog` plaintext finding — new, real, out of scope here.** See register **C8**: `GET /api/queries/:id/emails` returns the rendered forwarder-invitation email (including a working portal link) to any authenticated user, not just Managers/Admins. Predates this branch entirely; not an S5.9 consequence, and not fixed in this sub-build. Flagged for your priority call.

### Register housekeeping
**B1, B2 and B3 close** (see register section D for exactly how each was closed). **A5 stays open by choice** — S5.9 built and rebuilt the entire approval flow without touching query-level status; a query still reads plain "Quoted" throughout send/approve/reject. **C8 is new** (the `MessageLog` finding above). C5 (the intermittent e2e flake) was watched, not reproduced, on this session's full-gate run.

### Outcome
- **`pnpm run ci` GREEN**, first try, no flake this session: shared 26 files/390 tests · web 113 files/781 tests · api 93 suites/442 tests · lint + typecheck + 3 builds clean.
