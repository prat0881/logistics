...
- ✅ **Master-data consistency pass — 13 tasks complete** on branch `claude/master-data-consistency-a920af` (`.superpowers/sdd/2026-08-31-master-data-consistency/`). Fixed silent-save and role-gating defects the whole-branch review found (see above), rebuilt the FreightForwarder form's contacts UI to full edit/delete, added FX-rate zoned-datetime handling, and — the final task — restricted Charge Catalogue's `inputType` to the two values `resolveChargeConfig` actually resolves (`PLAIN`, `HEAVY_WEIGHT_CALC`) and removed `sortOrder` from the admin UI entirely (the API still accepts it; nothing sends it any more). No API/schema change in this pass. **The deferred Stage-4 pass (below) is still next** — none of its seven items were touched. **The post-deploy corrections (below) are still owed** and now apply through the rebuilt FF form rather than the original one.
