...
- **Lint is part of verification** (CI runs `pnpm run lint`): run `pnpm run lint` (all packages) before the final commit/PR. Watch `@typescript-eslint/no-unused-vars` — do not create unused bindings (e.g. destructure-to-omit `const { x, ...rest }`; use `{ ...obj, x: undefined }` instead).
- **Fresh-worktree setup:** a new worktree needs `apps/api/.env` copied from the main checkout (gitignored → provides `DATABASE_URL`) and `pnpm exec prisma generate` before api tests can boot; build `@svyft/shared` once (`pnpm --filter @svyft/shared build`).
