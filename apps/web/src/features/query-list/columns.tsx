...
/**
 * Factory: build the query-list column definitions.
 *
 * - `queryDate` and `updatedAt` display in the **viewer's local timezone** (system zone).
 * - `responseDeadline` displays in the **org timezone** (`orgZone` parameter).
 *
 * All date cells include a short zone label via `formatInZone` (e.g. "GMT+5:30", "EDT").
 */
export function makeQueryColumns(orgZone: string): ColumnDef<QueryListRow>[] {
  ...
  return [
    ...
    },
    {
      ...
      cell: ({ getValue }) => (
        <span className="font-mono tabular-nums text-muted-foreground">
          {formatInZone(getValue<string>(), viewer)}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      cell: ({ getValue }) => (
        <span className="font-mono tabular-nums text-muted-foreground">
          {formatInZone(getValue<string | null>() ?? "", orgZone) || "—"}
        </span>
      ),
    },
    {
