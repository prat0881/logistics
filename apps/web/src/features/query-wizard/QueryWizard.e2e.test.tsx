...
// ── Scenario 4: optional-gaps dialog appears when checklist has unchecked items ──
describe("QueryWizard e2e — optional-gaps dialog", () => {
  it("shows CreateQueryDialog when there are unchecked checklist items but no blocking findings", async () => {
    const user = userEvent.setup();
    const createCalls: string[] = [];

    // Detail with all required fields filled but checklist items unchecked
    ...
    vi.stubGlobal(
      ...
      mockFetch((url, init) => {
        ...
        if (url.includes(`/api/queries/${Q_ID}`))
          return { status: 200, body: detailWithChecklist };
    ...
    // The optional-gaps dialog should appear
    await waitFor(() =>
      expect(
        screen.getByText(/Create query with missing optional info/i),
      ).toBeInTheDocument(),
    );

    // The unchecked item label should appear in the dialog body (may also be in checklist)
    expect(screen.getAllByText("Weight confirmed").length).toBeGreaterThanOrEqual(1);

    // Clicking "Send Anyway" should fire the create call
    const sendBtn = screen.getByRole("button", { name: /Send Anyway/i });
  ...
  it("Cancel from the optional-gaps dialog does NOT fire /create", async () => {
    const user = userEvent.setup();
    const createCalls: string[] = [];

    const detailWithChecklist = {
      ...fullDraftDetail,
      checklist: [{ id: "c1", itemKey: "weight-confirmed", checked: false }],
    };
    ...
    // Dialog should close, /create NOT called
    await waitFor(() =>
