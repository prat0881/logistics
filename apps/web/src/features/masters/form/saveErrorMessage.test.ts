...
describe("saveErrorMessage", () => {
  it("prefers the first usable issue message on an ApiError with issues", () => {
    ...
    expect(saveErrorMessage(err, "Could not save this client")).toBe(
  ...
  it("falls through past an issue whose message is blank or missing to the next usable one", () => {
    ...
    expect(saveErrorMessage(blankFirst, "fallback")).toBe("Required");

    const missingFirst = new ApiError(400, "Validation failed", undefined, [
      { path: ["contacts"] },
      { path: ["city"], message: "Required" },
    ]);
    expect(saveErrorMessage(missingFirst, "fallback")).toBe("Required");
  });

  it("uses err.message when the ApiError has no issues", () => {
    const err = new ApiError(409, "This client already has a primary contact");
    expect(saveErrorMessage(err, "fallback")).toBe("This client already has a primary contact");
  });

  it("uses the fallback when the ApiError's message is blank and there are no issues", () => {
    const err = new ApiError(500, "   ");
    expect(saveErrorMessage(err, "Could not save this client")).toBe("Could not save this client");
  });
  ...
  it("uses the fallback for a non-ApiError value", () => {
    expect(saveErrorMessage(new Error("network down"), "Could not save this client")).toBe(
      "Could not save this client",
    );
    expect(saveErrorMessage("some string", "Could not save this client")).toBe(
      "Could not save this client",
    );
    expect(saveErrorMessage(undefined, "Could not save this client")).toBe(
      "Could not save this client",
    );
  });
});
