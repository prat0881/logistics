...
// (a different mode) to prove the popover mode-filters the catalogue against leg.mode.
...
describe("ConfigureChargesPopover", () => {
  it("is empty by default, and opening shows Standard/Tag-driven headings + read-only cores note, mode-filtered", async () => {
    stubPatch();
    wrap(<ConfigureChargesPopover queryId={QUERY_ID} leg={leg()} catalogue={CATALOGUE} disabled={false} />);
    ...
    // Cores render as a read-only note, not a selectable checkbox.
    expect(screen.getByText(/Always included: Air Zone 1/)).toBeInTheDocument();
    expect(screen.queryByRole("checkbox", { name: /air zone 1/i })).toBeNull();

    // Mode filter: the SEA-mode line is excluded for this AIR leg.
    expect(screen.queryByText("Sea Only Line")).toBeNull();

    // Only the 2 STANDARD + 1 TAG_DRIVEN rows are checkboxes (core is excluded).
  ...
  it("omits a section heading entirely when the mode-filtered catalogue has no rows for that role", async () => {
    ...
    wrap(<ConfigureChargesPopover queryId={QUERY_ID} leg={leg()} catalogue={noTagDriven} disabled={false} />);
  ...
  it("toggling a checkbox on calls PATCH …/legs/:id with the new id set, and updates the trigger count", async () => {
    ...
    expect(screen.getByRole("button", { name: /configure charges \(1\)/i })).toBeInTheDocument();
  ...
  it("disabled renders every checkbox read-only and blocks the API call on click", async () => {
    const fetchMock = stubPatch();
    ...
    expect(screen.getByRole("button", { name: /configure charges \(0\)/i })).toBeInTheDocument();
  });
});
