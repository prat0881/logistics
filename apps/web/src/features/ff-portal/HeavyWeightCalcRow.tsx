...
export function HeavyWeightCalcRow({ index, label }: { index: number; label: string }): JSX.Element {
  ...
  const amount = piece != null && limit != null && rate != null ? computeHeavyWeightAmount(piece, limit, rate) : null;
  return (
    <div className="space-y-2 rounded-md border border-border p-3">
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="space-y-1">
          ...
          <NumberField aria-label={`Piece weight (kg) for ${label}`} value={piece}
        ...
        <div className="space-y-1">
          ...
          <NumberField aria-label={`Airline limit (kg) for ${label}`} value={limit}
        ...
        <div className="space-y-1">
          ...
          <NumberField aria-label={`Rate per excess kg for ${label}`} value={rate}
      ...
      <div className="flex justify-between text-sm" data-testid={`hwc-amount-${index}`}>
        ...
        <span className="font-mono tabular-nums font-medium">{amount == null ? "—" : fmtAmount(amount)}</span>
      </div>
    </div>
  );
}
