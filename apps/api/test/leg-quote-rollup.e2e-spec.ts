...
describe("Leg rollup from quotes (e2e)", () => {
  ...
  beforeAll(async () => {
    ...
    prisma = moduleRef.get(PrismaService); status = moduleRef.get(StatusService);
    const query = await prisma.query.create({ data: { queryCode: "YAL00-ROLLUPTEST" } }); queryId = query.id;
