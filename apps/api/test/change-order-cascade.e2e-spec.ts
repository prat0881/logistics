...
// SAME mediated service boundary the controllers use (CargoService.update — not ChangeMediator
// directly), plus the Task 11 re-distribute reactivation. Two distributed legs prove design
// §11.3 "minimal blast radius": L1 (cargo C1, FF-A QUOTED + FF-B RFQ_SENT) is the edited leg;
// L2 (cargo C2, FF-C QUOTED) is the untouched sibling — C2 is assigned ONLY to L2 (never L1),
// so the classifier's cargo→legs fan-out (LegCargo-driven, [impact.classifier.ts]) must never
...
//
// Story: (1) edit C1.grossWt WITHOUT a reason → 409 ConflictException, preview names FF-A
...
const CARGO2_GROSS_WT = 60; // kg — L2's cargo; must never move
...
describe("Change-order cascade — capstone full flow (e2e)", () => {
  ...
  // Restrict) → query (cascades points/legs/cargo/legCargo) → FFs swept last.
  const cleanup = async () => {
  ...
  beforeAll(async () => {
    ...
    cargoService = moduleRef.get(CargoService);
  ...
  const mkSnapshot = (legId: string, cargoItemId: string, grossWt: number) => ({
    legId,
    frozenAt: "2020-01-01T00:00:00.000Z",
    cargo: [{ cargoItemId, grossWt: String(grossWt) }],
  });
  ...
  it("full field-edit cascade: reject without reason, apply+cascade with reason, minimal blast radius, re-distribute reactivates", async () => {
    ...
    //   L1 — cargo C1, FF-A (QUOTED) + FF-B (RFQ_SENT)   ← the edited leg
    //   L2 — cargo C2, FF-C (QUOTED)                      ← sibling that MUST stay untouched
    // ─────────────────────────────────────────────────────────────────────────
    ...
    const leg1 = await prisma.leg.create({
      data: {
        ...
        legCargo: { create: { cargoItemId: cargo1.id } }, // ONLY C1 — drives the classifier's fan-out
      },
    });
    const leg2 = await prisma.leg.create({
      data: {
        ...
        readyDate: new Date(),
        targetDelivery: new Date(Date.now() + 7 * 86400000),
        legCargo: { create: { cargoItemId: cargo2.id } }, // ONLY C2 — never C1
      },
    });
    ...
    const quoteA = await prisma.quote.create({
      data: {
        ...
        manifestSnapshot: mkSnapshot(leg1.id, cargo1.id, OLD_GROSS_WT) as unknown as Prisma.InputJsonValue,
      },
    });
    // FF-A's pricing child — must SURVIVE invalidation (non-destructive history, design §11).
    await prisma.quoteCargoLine.create({
      data: { quoteId: quoteA.id, cargoItemId: cargo1.id, freightDensity: 167, chargeableWeightT: 1.5 },
    });
    ...
    const quoteC = await prisma.quote.create({
      data: {
        ...
        manifestSnapshot: mkSnapshot(leg2.id, cargo2.id, CARGO2_GROSS_WT) as unknown as Prisma.InputJsonValue,
      },
    });

    // ─────────────────────────────────────────────────────────────────────────
    // Step 1 — WITHOUT a reason: the mediated cargo update rejects with a 409-shaped
    // ConflictException carrying needsChangeOrder + the blast-radius preview, and applies
    // NOTHING. FF-A/FF-B are the only quotes previewed; L2/FF-C never appear.
    // ─────────────────────────────────────────────────────────────────────────
    let caught: unknown;
    try {
      await cargoService.update(query.id, cargo1.id, { grossWt: NEW_GROSS_WT }, user);
    } catch (err) {
      caught = err;
    }
    ...
    const cargo1AfterPreview = await prisma.cargoItem.findUnique({ where: { id: cargo1.id } });
    expect(Number(cargo1AfterPreview?.grossWt)).toBe(OLD_GROSS_WT);
    ...
    // ─────────────────────────────────────────────────────────────────────────
    await cargoService.update(query.id, cargo1.id, { grossWt: NEW_GROSS_WT, reason: REASON }, user);

    const cargo1After = await prisma.cargoItem.findUnique({ where: { id: cargo1.id } });
    expect(Number(cargo1After?.grossWt)).toBe(NEW_GROSS_WT);
    ...
    // FF-B (RFQ_SENT) — refreshed in place to the new weight, still pending.
    ...
    const ffCSnap = quoteCAfter?.manifestSnapshot as unknown as {
      cargo: { grossWt: string; cargoItemId: string }[];
    };
    expect(ffCSnap.cargo[0].grossWt).toBe(String(CARGO2_GROSS_WT)); // NOT refreshed, NOT touched
    expect(ffCSnap.cargo[0].cargoItemId).toBe(cargo2.id);
