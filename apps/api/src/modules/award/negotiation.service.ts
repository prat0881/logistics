...
import { AwardDecisionStatus, type Quote } from "@prisma/client";
import {
  LegEvent,
  LegStatus,
  QuoteEvent,
...
export class NegotiationService {
  ...
  async requestRequote(
    ...
    // 2) Decision: reset to a clean DRAFT slate (the basis changed) + audit event. Bundled in
    // one transaction since both are plain writes with no external side effects (mirrors
    // AwardService.shortlist's upsert+event pattern).
    ...
    // 3) Leg: only reopen if the negotiated quote had actually carried an approval — a leg
    ...
    // is not in its RESOLVED set, so it never fires a forward transition off this event).
    if (wasApproved) {
