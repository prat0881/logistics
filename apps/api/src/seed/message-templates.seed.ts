...
export const MESSAGE_TEMPLATES: {
  ...
  // decision back to DRAFT), but nothing announced it. Recipient is the query's assigned user,
  // with an all-active-Executives broadcast only as the fallback (award.service.ts's reject();
  // an earlier version broadcast unconditionally on the false premise that this schema has no
  // assignment concept). IN_APP only: the executive is inside the app, and there is no
  // forwarder-facing side to this event.
  {
