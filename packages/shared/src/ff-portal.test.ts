...
const valid: QuoteDraft = {
  ...
  charges: [
    { zone: "ORIGIN", presetKey: "AIR_ORIGIN_THC", label: "Origin THC", amount: 100, note: "x" },
  ],
  trucking: [],
  seaRates: [],
  warehouse: [],
...
describe("quoteDraftSchema", () => {
  it("accepts a well-formed QuoteDraft (nullable amounts allowed)", () => {
    ...
    const partial = {
      ...valid,
      charges: [{ zone: "ORIGIN", presetKey: null, label: "x", amount: null }],
    };
  ...
  it("rejects a malformed body (bad zone / wrong type)", () => {
    expect(
      quoteDraftSchema.safeParse({
        ...valid,
        charges: [{ zone: "NOPE", label: "x", amount: 1, presetKey: null }],
      }).success,
    ).toBe(false);
